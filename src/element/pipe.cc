	...
	const Material<T> *prop, const Section<T> *sect, const int *opt)
{
	...
	U Ro = 0.5*sect->get_sect_prop(SectionProp::OD);//! Outer radius.
	decltype(Ro) Ri  = Ro - sect->get_sect_prop(SectionProp::TKWALL);//! Inner radius.
	decltype(Ro) Ax  = PI<U>()*(Ro*Ro - Ri*Ri);//! Area of cross-section.
	decltype(Ro) Jxx = 0.5*PI<U>()*(pow(Ro, 4.0) - pow(Ri, 4.0));
	decltype(Ro) Izz = 0.5*Jxx;
	decltype(Ro) Iyy = Izz;
	decltype(Ro) v   = prop->get_material_prop(MaterialProp::PRXY);
	decltype(Ro) ES  = prop->get_material_prop(MaterialProp::YOUNG)*Ax;
	decltype(Ro) GJx = prop->get_material_prop(MaterialProp::YOUNG)*Jxx*0.5/(1. + v);
	//! Coordinate transform.
	decltype(Ro) Le;//! Lenght of pipe.
	matrix_<U> tt    = matrix_<U>::Zero(3, 3);
	std::tie(Le, tt) = NodeFunc<T, U>::coord_tran(p1, p2);
	...
	decltype(Ro) alpha = 2.0;
	decltype(Ro) Ksy   = 2.4e1*(1. + v)*Iyy*alpha/(Ax*Le*Le);
	decltype(Ro) Ksz   = Ksy;
	// double Ksz{2.4e1*(1.+v)*Izz*alpha/(Ax*Le*Le)};
	decltype(Ro) EIz = prop->get_material_prop(MaterialProp::YOUNG)*Izz;
	...
	decltype(Ro) rz = prop->get_material_prop(MaterialProp::DENS)*Izz;
	...
	decltype(Ro) SIF = U(1);//! Stress intensity factor.

	std::map<std::string, U> attr{{"Length", Le}, {"Area", Ax}, {"Volume", Ax*Le}, {"Mass", Me},
	 	{"Aw", Ax}, {"Thick", Ro-Ri}, {"OuterDiameter", U(2)*Ro}, {"InnerDiameter", U(2)*Ri},
		{"Iy", Iyy}, {"Iz", Izz}, {"Jx", Jxx}, {"StressIntensificationFactor", SIF},
		{"InternalPressure", sect->get_sect_prop(SectionProp::PRESIN)}, {"Angle", U(-1)},
		{"CurvatureRadius", U(-1)}, };
	...
	const NodeBase<T> *cen, const Material<T> *prop, const Section<T> *sect)
{
	...
	decltype(Ro) Ri  = Ro - sect->get_sect_prop(SectionProp::TKWALL);
	decltype(Ro) t   = sect->get_sect_prop(SectionProp::TKWALL);
	decltype(Ro) Ax  = PI<U>()*(Ro*Ro - Ri*Ri);
	decltype(Ro) Jxx = 0.5*PI<U>()*(pow(Ro, 4.0) - pow(Ri, 4.0));
	decltype(Ro) Iyy = 0.5*Jxx;
	decltype(Ro) R   = sect->get_sect_prop(SectionProp::RADCUR);
	decltype(Ro) R2  = R*R;
	decltype(Ro) R3  = R2*R;
	decltype(Ro) v   = prop->get_material_prop(MaterialProp::PRXY);
	...
	decltype(Ro) la = vxx.norm(), lb = vxy.norm();
	vxx /= vxx.norm();
	vzz = vxx.cross(vxy);
	vzz /= vzz.norm();
	vyy = vzz.cross(vxx);
	...
	decltype(Ro) cos_2the = cos(2.0*the), sin_2the = sin(2.0*the);
	...
	decltype(Ro) EA = prop->get_material_prop(MaterialProp::YOUNG)*Ax;
	decltype(Ro) GA = prop->get_material_prop(MaterialProp::YOUNG)*Ax*.5/(1.0 + v);
	decltype(Ro) EI = prop->get_material_prop(MaterialProp::YOUNG)*Iyy;
	...
	decltype(Ro) cos_b = cos(.5*the), sin_b = sin(.5*the);
