 ...
 */
template <class T, typename U>
void Element<T, U>::post_stress(const vecX_<T> x) {
	switch (this->etype_) {
 ...
 */
template <class T, typename U>
std::array<size_t, 2> Element<T, U>::get_rhs_shape() const {
	std::array<size_t, 2> sz;
 ...
 */
template <class T, typename U>
std::array<size_t, 2> Element<T, U>::get_result_shape() const {
	std::array<size_t, 2> sz;
 ...
 */
template <class T, typename U>
void Element<T, U>::set_element_type(int x) {
	switch (x) {
		case 14:
