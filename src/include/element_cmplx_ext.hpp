...
void Element<T>::form_matrix(const Node<U, T> p[], const Material<U> *matl, const Section<U> *sect,
	const std::vector<LoadCell<T>> load)
{
	...
	case ElementType::PIPE16:
		std::tie(this->stif_,
			...
			this->rhs_,
			this->attr_,
			this->rhs_cmplx_) = StructuralElement<U, T>::pipe16(&p[0], &p[1], matl, sect, opt.data(), load);
		break;
	case ElementType::PIPE18:
		std::tie(this->stif_,
			...
			this->rhs_,
			this->attr_,
			this->rhs_cmplx_) = StructuralElement<U, T>::pipe18(&p[0], &p[1], &p[2], matl, sect, load);
		break;
...
void Element<T>::form_matrix(const vector<Node<U, T>> pt, const Material<U> *mp,
	const Section<U> *sect, const std::vector<LoadCell<T>> load)
{
	auto opt = this->get_option();
	switch(this->etype_){
	case ElementType::PIPE16:
		std::tie(this->stif_,
			...
			this->rhs_,
			this->attr_,
			this->rhs_cmplx_) = StructuralElement<U, T>::pipe16(pt, mp, sect, opt.data(), load);
		break;
	case ElementType::PIPE18:
		std::tie(this->stif_,
			this->mass_,
			...
			this->rhs_,
			this->attr_,
			this->rhs_cmplx_) = StructuralElement<U, T>::pipe18(pt, mp, sect, load);
		break;
	case ElementType::MASS21:
		std::tie(this->stif_,
			this->mass_,
