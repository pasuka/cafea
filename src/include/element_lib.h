...
struct StructuralElement
{
	 ...
	 */
	static elem_out_5<U> pipe16(const NodeBase<T>*, const NodeBase<T>*, const Material<T>*,
		const Section<T>*, const int*);

	static elem_out_5<U> pipe16(const std::vector<Node<T, U>>, const Material<T>*, const Section<T>*,
		const int*);
	/**
	 *  \brief 2-node elbow pipe element.
	 */
	static elem_out_5<U> pipe18(const NodeBase<T>*, const NodeBase<T>*, const NodeBase<T>*,
	 ...
	 */
	static elem_out_5<U> mass21(const NodeBase<T>*, const Material<T>*, const Section<T>*,
		const int[]);
	static elem_out_5<U> mass21(const std::vector<Node<T, U>>, const Material<T>*, const Section<T>*,
		const int[]);
	/**
	 *  \brief 2-node spring element.
	 */
	static elem_out_5<U> combin14(const NodeBase<T>*, const NodeBase<T>*, const Material<T>*,
		const Section<T>*, const int[]);
	static elem_out_5<U> combin14(const std::vector<Node<T, U>>, const Material<T>*, const Section<T>*,
		const int[]);
	/**
	 *  \brief 8-node Mindlin shell element.
	 */
	static elem_out_5<U> shell8r(
...
struct StructuralElementPost
{
	...
	static matrix_<T> pipe(const matrix_<T> stif, const matrix_<T> tran, const matrix_<T> x,
		const matrix_<T> rhs, const std::map<std::string, T> attr, bool is_pres=false, T pres_in=T(0), T pres_out=T(0));
	/**
	 *  \brief 2-node straight/elbow pipe element post process in complex domain.
	 */
	static cmatrix_<T> pipe_cmplx(const matrix_<T> stif, const matrix_<T> tran, const cmatrix_<T> x,
