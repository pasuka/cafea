...
void SolutionStatic<FileReader, Scalar, ResultScalar>::load(const char* fn) {
	...
	if (is_cdb()) {
		...
		wrapper_::model_data_ptr(&p_node, &p_elem, &p_matl, &p_real, &a1, &a2, &a3, &a4);
				...
				if (std::any_of(tmp.cbegin(), tmp.cend(), [] (int i) { return i < 0;})) {
					for (size_t j = 0; j < tmp.size(); j++) {
						if (tmp[j] < 0) {
							switch (j) {
								case 0:
									this->bc_group_.push_back({p_node->id_, BoundaryType::FIXED, DofLabel::UX});
									break;
								case 1:
									this->bc_group_.push_back({p_node->id_, BoundaryType::FIXED, DofLabel::UY});
									break;
								case 2:
									this->bc_group_.push_back({p_node->id_, BoundaryType::FIXED, DofLabel::UZ});
									break;
								case 3:
									this->bc_group_.push_back({p_node->id_, BoundaryType::FIXED, DofLabel::URX});
									break;
								case 4:
									this->bc_group_.push_back({p_node->id_, BoundaryType::FIXED, DofLabel::URY});
									break;
								case 5:
									this->bc_group_.push_back({p_node->id_, BoundaryType::FIXED, DofLabel::URZ});
									break;
								case 6:
									this->bc_group_.push_back({p_node->id_, BoundaryType::FIXED, DofLabel::WARP});
									break;
								default: fmt::print("Only support 6 dofs now\n");
							}
						}
					}
		...
		if (is_bcy()) {
			...
			wrapper_::model_data_ptr_bcy(&p_node, &p_elem, &p_matl, &p_sect, &p_bndy, &p_load, &a1, &a2, &a3, &a4, &a5, &a6);
			assert(a1 > 0);
			assert(a2 > 0);
			assert(a3 > 0);
			assert(a4 > 0);
...
void SolutionStatic<FileReader, T, U>::solve() {
	this->solver_->load(this->mat_pair_.get_stif_ptr(), this->mat_pair_.get_coord_ptr(),
		this->mat_pair_.get_nnz(), this->mat_pair_.get_dim());

	bool flag = this->solver_->solve(this->mat_pair_.get_rhs_ptr(), this->mat_pair_.get_dim());
	if (!flag) {
