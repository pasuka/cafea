			...
			for (auto ja = 0; ja < ndof; ja++) {
				...
				for (size_t ib = 0; ib < nn; ib++) {
					...
					for (auto jb = 0; jb < ndof; jb++) {
						...
						this->mat_pair_.add_matrix_data_KM(va[ja], vb[jb], p_stif(row_, col_), p_mass(row_, col_));
					}
				}
			}
		}
...
void SolutionModal<FileReader, Scalar, ResultScalar>::solve() {
	...
	this->solver_->load(this->mat_pair_.get_stif_ptr(), this->mat_pair_.get_mass_ptr(), this->mat_pair_.get_coord_ptr(), nnz, dim);
...
void SolutionModal<FP, T, U>::set_parameter(SolutionOption chk, init_list_<U> val) {
	...
	switch (chk) {
		case SolutionOption::MODAL_NUMBER:
			for (auto p: val) this->freq_num_ = static_cast<int>(p);
		...
		case SolutionOption::MODAL_FREQ_RANGE:
			this->freq_num_ = -1;
			for (auto p: val) {
				if (1 >= flag) this->freq_range_[flag] = p;
				flag++;
			}
			break;
