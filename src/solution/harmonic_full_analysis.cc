			...
			for (auto ja = 0; ja < ndof; ja++) {
				...
				for (auto ib = 0; ib < nn; ib++) {
					...
					for (auto jb = 0; jb < ndof; jb++) {
						...
						this->mat_pair_.add_matrix_data(va[ja], vb[jb], p_stif(row_, col_), p_mass(row_, col_));
					}
				}
			}
		}
		...
		for (auto &i: node_list) {
			...
			if (p_node.is_activated()) {
				for (int j = 0; j < num_step; j++) {
					// fmt::print("Col index:{}\n", j*int(nn)+iter);
					matrix_<COMPLEX<U>> yy = stress.block(43, j*static_cast<int>(nn)+iter, 8, 1);
...
void SolutionHarmonicFull<FileReader, Scalar, ResultScalar>::write2mat(const char* fname, bool is_ver73) {
	...
	for (const auto &it: this->elem_group_) {
		...
		val[0] = static_cast<double>(it.second.get_element_id());
		val[1] = static_cast<double>(it.second.get_element_type_id());
		val[2] = static_cast<double>(it.second.get_material_id());
		val[3] = static_cast<double>(it.second.get_section_id());
		...
		for (size_t i = 0; i < dim_vec[0]; i++) nodes_d[i] = static_cast<double>(nodes[i]);
		matvar[4] = Mat_VarCreate(fieldnames[4], MAT_C_DOUBLE, MAT_T_DOUBLE, 2, dim_vec, nodes_d, 0);
		...
		matvar[8] = Mat_VarCreate(fieldnames[8], MAT_C_DOUBLE, MAT_T_DOUBLE, 2, dim_rhs, &c_rhs, MAT_F_COMPLEX);
		...
		if (1 > stress.rows() && 1 > stress.cols()) {
			...
			matvar[9] = Mat_VarCreate(fieldnames[9], MAT_C_DOUBLE, MAT_T_DOUBLE, 2, dim_stress, &c_stress, MAT_F_COMPLEX);
		}
	...
	for (const auto &it: this->node_group_) {
		...
		val[0] = static_cast<double>(p_node.get_id());
		val[1] = 0.0;
		for (size_t i = 0; i < 2; i++) {
			matvar[i] = Mat_VarCreate(fieldnames2[i], MAT_C_DOUBLE, MAT_T_DOUBLE, 2, dim1x1, &val[i], 0);
		}
		xyz[0] = static_cast<double>(p_node.get_x());
		xyz[1] = static_cast<double>(p_node.get_y());
		xyz[2] = static_cast<double>(p_node.get_z());
		matvar[2] = Mat_VarCreate(fieldnames2[2], MAT_C_DOUBLE, MAT_T_DOUBLE, 2, dim3x1, xyz, 0);
		if (p_node.is_activated()) {
			//
			matrix_<COMPLEX<ResultScalar>> rst = p_node.template get_result<COMPLEX<ResultScalar>>(SolutionType::HARMONIC_FULL,
				LoadType::STRESS, -1);
			size_t sz[2] = {1, 1};
	...
	matvar_t *gk = Mat_VarCreate("stif", MAT_C_SPARSE, MAT_T_DOUBLE, 2, dim_vec, global_stif.get(), MAT_F_DONT_COPY_DATA);
	Mat_VarWrite(matfp, gk, MAT_COMPRESSION_ZLIB);
	Mat_VarFree(gk);
	global_stif.reset(nullptr);
	auto global_mass = this->mat_pair_.get_mass_mat();
	matvar_t *gm = Mat_VarCreate("mass", MAT_C_SPARSE, MAT_T_DOUBLE, 2, dim_vec, global_mass.get(), MAT_F_DONT_COPY_DATA);
...
void SolutionHarmonicFull<FP, T, U>::set_parameter(SolutionOption chk, bool val) {
	...
	switch (chk) {
		case SolutionOption::LUMPED_MASS: this->set_mass_lumped(val); break;
		case SolutionOption::PRESSURE_INTERNAL: this->has_pressure_ = val; break;
