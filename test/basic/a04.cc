...
TEST_CASE("init 02", "[Material]") {
    ...
    for (int x: {1, 2, 3, 4}) {
        REQUIRE(mfa.get_material_prop(gen_material_prop(x)) == Approx(2.06e11f).epsilon(.1e-3f));
    }
    for (int x: {5, 6, 7}) {
        REQUIRE(mfa.get_material_prop(gen_material_prop(x)) == Approx(.94e10f).epsilon(.1e-3f));
    }
    for (int x: {8, 9, 10, 11, 12, 13, 14}) {
        REQUIRE(mfa.get_material_prop(gen_material_prop(x)) == Approx(.35f).epsilon(.1e-3f));
    }
